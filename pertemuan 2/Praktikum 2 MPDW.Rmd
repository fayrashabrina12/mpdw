...
```{r}
# Input Data IPM Semarang (2015-2024)
tahun <- 2015:2024
ipm <- c(80.23, 81.19, 82.01, 82.72, 83.19, 83.05, 83.55, 84.08, 84.43, 85.24)
...
Model regresi linear sederhana untuk memprediksi `ipm` berdasarkan `tahun`. Di tahap ini, kita berasumsi modelnya baik-baik saja.
```{r}
...
```
**Observasi Awal:**
**Nilai Adjusted R-squared sangat tinggi (0.9934)** dan koefisien `tahun` **sangat signifikan (p-value \< 0.001)**
...
-   P-value yang dihasilkan **sangat kecil yaitu 0.007478** di bawah tingkat signifikansi 0.05.
...
round(tab, 4)
```
Mencari perkiraan ρ yang menghasilkan SSE minimum. Hasil di atas terlihat ρ minimum ketika 0.4. Namun, hasil tersebut masih kurang teliti sehingga akan dicari kembali ρ yang lebih optimum dengan ketelitian yang lebih. Jika sebelumnya jarak antar ρ yang dicari adalah 0.1, kali ini jarak antar ρ adalah 0.001 dan dilakukan pada selang 0.3 sampai dengan 0.6.
...
```
input y, x dan rho maksimum yang udah dicoba 
```{r}
model_hl <- hildreth.lu(dt_ipm$ipm, dt_ipm$tahun, rho = 0.353)
summary(model_hl)
```
